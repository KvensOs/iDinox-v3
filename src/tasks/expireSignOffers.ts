import { Client, EmbedBuilder, TextChannel, Message, AttachmentBuilder } from "discord.js";
...
async function expireOne(client: Client, offer: SignOffer): Promise<void> {
    try {
        const channel = await client.channels.fetch(offer.channelId).catch(() => null) as TextChannel | null;

        if (channel?.isTextBased()) {
            const msg: Message | null = await channel.messages.fetch(offer.messageId).catch(() => null);
            ...
            if (msg?.editable && msg.embeds[0]) {
                ...
                const updated = EmbedBuilder.from(msg.embeds[0])
                    .setColor(0x95A5A6)
                    .spliceFields(5, 1, { name: "📋 Estado", value: "⌛ Vencida", inline: true })
                    .setFooter({ text: "Oferta vencida · iDinox v3" });

                if (logoExists && resolvedLogoFilename) updated.setThumbnail(`attachment://${resolvedLogoFilename}`);
        ...
        await offer.destroy();
        logger.info(`Offer #${offer.id} expired (target: ${offer.targetDiscordId}, modality: ${offer.modalityId})`);
    } catch (err) {
...
export function startExpireSignOffersTask(client: Client): void {
    ...
    logger.info(`expireSignOffers task started — interval: ${INTERVAL_MS / 1000}s`);
}