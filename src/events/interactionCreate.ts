...
import { Modality, DEFAULT_SETTINGS, ModalitySettings } from "../database/models/Modality.js";
...
async function handleSignButton(interaction: ButtonInteraction): Promise<void> {
    ...
    if ((action === "accept" || action === "reject") && who !== targetId) {
        await interaction.reply(ephemeral([errorEmbed("Solo el jugador al que se le envió la oferta puede responderla.")]));
        return;
    }
    ...
    if (action === "cancel" && who !== dtId) {
        ...
        const isMainDT = !!(settings.rol_dt && member?.roles.cache.has(settings.rol_dt));

        if (!isMainDT) {
            await interaction.reply(ephemeral([errorEmbed("Solo quien envió la oferta o el DT principal puede retirarla.")]));
            return;
        }
    }
    ...
    if (!modality || !team || !season) {
        await interaction.followUp(ephemeral([errorEmbed("Esta oferta ya no es válida (modalidad, equipo o temporada no encontrados).")]));
        return;
    }
    ...
    const displayTarget = targetUser?.globalName ?? targetUser?.username ?? targetId;
    ...
    const buildFiles = (): AttachmentBuilder[] => {
        ...
        if (fondoExists)
            files.push(new AttachmentBuilder(FONDO_PATH, { name: "fondo_fichajes.png" }));
        return files;
    };
    ...
    const buildUpdated = (color: number, estadoLabel: string, footer: string) => {
        const embed = EmbedBuilder.from(interaction.message.embeds[0])
            .setColor(color)
            .spliceFields(5, 1, { name: "📋 Estado", value: estadoLabel, inline: true })
            .setFooter({ text: footer })
            .setTimestamp();

        if (logoExists && resolvedLogoFilename) embed.setThumbnail(`attachment://${resolvedLogoFilename}`);
    ...
    if (action === "cancel") {
        const updated = buildUpdated(0xE67E22, "🚫 Oferta retirada", `Retirada por <@${who}> · iDinox v3 · ${modality.displayName}`);
        await Promise.all([
            interaction.editReply({ embeds: [updated], components: [], files: buildFiles() }),
            SignOffer.destroy({ where: { messageId } }),
        ]);
        await targetUser?.send({
            embeds: [new EmbedBuilder().setColor(0xE67E22).setTitle("Oferta retirada")
                .setDescription(`La oferta de **${team.name}** en **iDinox ${modality.displayName}** ha sido retirada.`)]
        }).catch(() => null);
        logger.info(`/market sign | RETIRADA | ${team.name} → ${displayTarget} en ${modality.displayName} | por <@${who}>`);
        return;
    }

    if (action === "reject") {
        const updated = buildUpdated(0xED4245, "❌ Oferta rechazada", `Rechazada · iDinox v3 · ${modality.displayName}`);
        await Promise.all([
            interaction.editReply({ embeds: [updated], components: [], files: buildFiles() }),
            SignOffer.destroy({ where: { messageId } }),
        ]);
        await dtUser?.send({
            embeds: [new EmbedBuilder().setColor(0xED4245).setTitle("Oferta rechazada")
                .setDescription(`**${displayTarget}** ha rechazado tu oferta para **${team.name}** en **iDinox ${modality.displayName}**.`)]
        }).catch(() => null);
        logger.info(`/market sign | RECHAZADA | ${team.name} → ${displayTarget} en ${modality.displayName}`);
        return;
    }

    if (!settings.marketOpen) {
        await interaction.followUp(ephemeral([errorEmbed("El mercado ya está cerrado. No se puede completar el fichaje.")]));
        return;
    }
    ...
    if (existingParticipant?.teamId) {
        await interaction.followUp(ephemeral([errorEmbed("Ya perteneces a un equipo en esta temporada. El fichaje no puede completarse.")]));
        return;
    }

    const roster = await Participant.count({ where: { teamId, seasonId: season.id, isActive: true } });
    if (roster >= modality.playersPerTeam) {
        await interaction.followUp(ephemeral([errorEmbed(`**${team.name}** ya tiene la plantilla completa (${roster}/${modality.playersPerTeam}).`)]));
        return;
    }

    const positionFinal = (position && position !== "N/A" ? position : (existingParticipant?.position ?? "N/A")) as Position;

    await Promise.all([
        existingParticipant
            ? (existingParticipant.teamId = teamId,
                existingParticipant.isActive = true,
                position && position !== "N/A" && (existingParticipant.position = position as Position),
                existingParticipant.save())
            : Participant.create({ playerId: targetPlayer.id, seasonId: season.id, modalityId, teamId, position: positionFinal, isActive: true }),
        SignOffer.destroy({ where: { messageId } }),
    ]);

    const targetMember = await interaction.guild?.members.fetch(targetId).catch(() => null);
    if (targetMember) {
        const displayName = targetUser?.globalName ?? targetUser?.username ?? targetId;
        await targetMember.setNickname(`#${team.abbreviation} ${displayName}`.slice(0, 32)).catch(() => null);
        const teamRole = interaction.guild?.roles.cache.get(team.roleId);
        if (teamRole) await targetMember.roles.add(teamRole).catch(() => null);
    }

    const updated = buildUpdated(0x57F287, "✅ Fichaje completado", `Aceptado · iDinox v3 · ${modality.displayName}`);
    await interaction.editReply({ embeds: [updated], components: [], files: buildFiles() });

    await dtUser?.send({
        embeds: [new EmbedBuilder().setColor(0x57F287).setTitle("✅ Fichaje aceptado")
            .setDescription(`**${displayTarget}** ha aceptado tu oferta y se une a **${team.name}** en **iDinox ${modality.displayName}**.`)
            .addFields({ name: "📍 Posición", value: `\`${positionFinal}\``, inline: true })]
    }).catch(() => null);

    logger.success(`/market sign | ACEPTADO | ${displayTarget} → **${team.name}** en ${modality.displayName} · pos: ${positionFinal}`);
}
        ...
        if (interaction.isButton() && interaction.customId.startsWith("sign_")) {
            await handleSignButton(interaction).catch(async (err) => {
                ...
                await interaction.followUp(ephemeral([
                    new EmbedBuilder().setColor(0xE74C3C).setTitle("Algo salió mal")
                        .setDescription("No se pudo procesar la respuesta. El equipo técnico fue notificado."),
                ])).catch(() => null);
            });
            return;
        }
