...
export default {
    ...
    data: new SlashCommandBuilder()
        ...
        .addStringOption(opt =>
            opt.setName("modalidad").setDescription("Modalidad en la que fichas.").setRequired(true).setAutocomplete(true)
        )
        .addUserOption(opt =>
            opt.setName("jugador").setDescription("Jugador al que quieres fichar.").setRequired(true)
        )
        .addStringOption(opt =>
            opt.setName("posicion").setDescription("Posición asignada (opcional).")
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        const posicion = interaction.options.getString("posicion") as Position | null;
        ...
        if (targetUser.id === interaction.user.id) {
            await interaction.editReply({ content: "No puedes enviarte una oferta a ti mismo." });
            return;
        }
        if (targetUser.bot) {
        ...
        try {
            ...
            if (!isDTorSubDT(member, settings)) {
                await interaction.editReply({
                    embeds: [new EmbedBuilder().setColor(0xE74C3C).setTitle("Sin permisos")
                        .setDescription("Solo los **Directores Técnicos** y **Sub-DT** pueden realizar fichajes.")],
                });
                return;
            }
            ...
            const [dtParticipant, targetParticipant] = await Promise.all([
                ...
                Participant.findOne({
                    where: { playerId: targetPlayer.id, seasonId: season.id, modalityId: modality.id },
            ...
            if (targetParticipant?.teamId) {
                await interaction.editReply({
                    embeds: [new EmbedBuilder().setColor(0xE74C3C).setTitle("Jugador no disponible")
                        .setDescription(
            ...
            const [team, plantillaActual, ofertaExistente] = await Promise.all([
                ...
                Participant.count({ where: { teamId: dtParticipant.teamId, seasonId: season.id, isActive: true } }),
                SignOffer.findOne({
                    where: { targetDiscordId: targetUser.id, modalityId: modality.id, status: "pending" },
                }),
            ]);
            ...
            if (plantillaActual >= modality.playersPerTeam) {
                await interaction.editReply({
                    embeds: [new EmbedBuilder().setColor(0xE74C3C).setTitle("Plantilla completa")
                        .setDescription(
            ...
            if (ofertaExistente) {
                await interaction.editReply({
                    embeds: [new EmbedBuilder().setColor(0xE67E22).setTitle("Oferta ya enviada")
                        .setDescription(
            ...
            if (!canalId) {
                await interaction.editReply({
                    content: "El canal de fichajes no está configurado. Usa `/setup` para configurarlo.",
                });
                return;
            }
            ...
            if (!canal) {
                await interaction.editReply({ content: "No se encontró el canal de fichajes configurado." });
                return;
            }
            ...
            const displayDT = interaction.user.globalName ?? interaction.user.username;
            ...
            const colorEmbed = (rolEquipo?.color && rolEquipo.color !== 0) ? rolEquipo.color : 0x1E90FF;
            ...
            const ofertaEmbed = new EmbedBuilder()
                ...
                .setFooter({ text: `Tienes ${OFFER_TTL_LABEL} para decidir · iDinox v3 · ${modality.displayName}` })
                .setTimestamp();

            if (logoExists && resolvedLogoFilename) ofertaEmbed.setThumbnail(`attachment://${resolvedLogoFilename}`);
                ...
                targetUser.send({
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setFooter({ text: `Tienes ${OFFER_TTL_LABEL} para decidir · iDinox v3` })
                            .setTimestamp(),
                    ],
                }).catch(() => null),
            ]);
            ...
            targetUser.send({
                embeds: [
                    new EmbedBuilder()
                        .setColor(colorEmbed)
                        .setDescription(`🔗 [Ver oferta en el canal de fichajes](${msgLink})`),
                ],
            }).catch(() => null);
            ...
            await interaction.editReply({
                embeds: [new EmbedBuilder().setColor(0xE74C3C).setTitle("Algo salió mal")
                    .setDescription("No se pudo enviar la oferta. El equipo técnico fue notificado.")],
            });
        }
    },
};