...
interface CompareSubject {
    player: Player;
    stats: StatValues;
    titles: number; // awards individuales ganados en el scope
}
...
const EMPTY_STATS: StatValues = { goles: 0, asistencias: 0, vallas: 0, autogoles: 0 };

// ─── Helpers ──────────────────────────────────────────────────────────────────

/** Suma stats de múltiples registros en un único StatValues. */
...
/**
 * Retorna los IDs de Participant de un jugador en una modalidad,
 * opcionalmente filtrados por temporada.
 */
...
/**
 * Carga stats de un jugador dentro del scope dado.
 * competitionId = undefined → todas las competencias de los participants.
 */
async function loadStats(
    ...
    const where: Record<string, unknown> = { participantId: { [Op.in]: participantIds } };
...
/**
 * Cuenta títulos individuales de un jugador (AwardWinner con playerId)
 * dentro del scope de temporada/competencia dado.
 */
async function countIndividualTitles(
    ...
    // Contar AwardWinner donde este jugador ganó
    // Necesitamos el Player.id, no el discordId
...
/**
 * Devuelve las temporadas donde AMBOS jugadores tienen Participant en la modalidad.
 */
async function getCommonSeasons(
    ...
    const [p1, p2] = await Promise.all([
        Participant.findAll({ where: { playerId: playerId1, modalityId }, attributes: ["seasonId"] }),
        Participant.findAll({ where: { playerId: playerId2, modalityId }, attributes: ["seasonId"] }),
    ]);

    const ids1 = new Set(p1.map(p => p.seasonId));
    const commonIds = [...new Set(p2.map(p => p.seasonId))].filter(id => ids1.has(id));
...
/**
 * Devuelve las competencias donde AMBOS jugadores tienen Stats en una temporada.
 */
async function getCommonCompetitions(
    ...
    const [stats1, stats2] = await Promise.all([
        Stat.findAll({ where: { participantId: { [Op.in]: ids1 } }, attributes: ["competitionId"] }),
        Stat.findAll({ where: { participantId: { [Op.in]: ids2 } }, attributes: ["competitionId"] }),
    ]);

    const compIds1 = new Set(stats1.map(s => s.competitionId));
    const commonIds = [...new Set(stats2.map(s => s.competitionId))].filter(id => compIds1.has(id));
    if (!commonIds.length) return [];
...
/** Padding derecho para alinear columnas en bloque de código. */
...
function buildEmbed(
    ...
    statLines.push(`${pad("G+A", COL_LABEL)}  ${pad(String(gaA) + gaMarkA, COL_VAL)}  ${gaB}${gaMarkB}`);
...
export default {
    ...
    emoji: "⚖️",
    usage: "/league-compare jugador1:@A jugador2:@B modalidad:[mod] [temporada] [competencia]",
    ...
    async autocomplete(interaction: AutocompleteInteraction): Promise<void> {
        ...
        // Para temporada y competencia necesitamos ambos jugadores y la modalidad
        const opts = interaction.options as unknown as ChatInputCommandInteraction["options"];
        ...
        if (focused.name === "temporada") {
            const seasons = await getCommonSeasons(player1.id, player2.id, modality.id);
        ...
        if (focused.name === "competencia") {
            ...
            const competitions = await getCommonCompetitions(player1.id, player2.id, modality.id, seasonId);
            return interaction.respond(
                competitions.map(c => ({
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        try {
            ...
            // Verificar que si hay temporada, ambos tengan participación
                ...
                if (!p1ids.length || !p2ids.length) {
                    ...
                    await interaction.editReply({
                        embeds: [new EmbedBuilder().setColor(0xE67E22)
                            .setDescription(`**${missing}** no tiene participación en esa temporada.`)],
                    });
                    return;
                }
            }
            ...
            // Construir scope label
            let scopeLabel = "_Todas las temporadas_";
            if (competitionId !== undefined) {
                const comp = await Competition.findByPk(competitionId);
                scopeLabel = comp ? `Competencia: \`${comp.name}\`` : `Competencia #${competitionId}`;
            } else if (seasonId !== undefined) {
                const season = await Season.findByPk(seasonId);
                scopeLabel = season ? `Temporada: \`${season.name}\`` : `Temporada #${seasonId}`;
            }

            const subject1: CompareSubject = { player: player1, stats: stats1, titles: titles1 };
            const subject2: CompareSubject = { player: player2, stats: stats2, titles: titles2 };
            ...
            logger.error(`/league-compare | ${user1.username} vs ${user2.username}`, error);
            await interaction.editReply({
                embeds: [new EmbedBuilder().setColor(0xE74C3C)
                    .setDescription("No se pudo cargar la comparativa. El equipo técnico fue notificado.")],
            });
        }
    },
};