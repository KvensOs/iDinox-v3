...
function buildEmbed(
    ...
    const filtroLabel = POSICIONES.find(p => p.value === filtroPos)?.label ?? "Todas las posiciones";
    ...
    return new EmbedBuilder()
        ...
        .setFooter({ text: `Página ${pagina + 1} de ${totalPags} · 📊 = tiene historial de stats` })
        .setTimestamp();
}

function buildComponents(lista: AgentEntry[], pagina: number): ActionRowBuilder<ButtonBuilder | StringSelectMenuBuilder>[] {
    const totalPags = Math.max(1, Math.ceil(lista.length / POR_PAGINA));
...
export default {
    ...
    data: new SlashCommandBuilder()
        ...
        .setDescription("Muestra los jugadores sin equipo disponibles en el mercado.")
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        try {
            ...
            if (!agentParticipants.length) {
                await interaction.editReply({
                    embeds: [new EmbedBuilder().setColor(0xF1C40F).setTitle("🕵️ Agentes Libres")
                        .setDescription(`No hay agentes libres en **${modality.displayName}** esta temporada.`)],
                });
                return;
            }
            ...
            const getFiltered = () => filtroPos === "ALL"
                ? entries
            ...
            await interaction.editReply({
                embeds: [new EmbedBuilder().setColor(0xE74C3C).setTitle("Algo salió mal")
                    .setDescription("No se pudo cargar la lista de agentes. El equipo técnico fue notificado.")],
            });
        }
    },
};