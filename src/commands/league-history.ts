...
type AwardFull = Award & {
  winners: AwardWinnerFull[];
  competition?: Competition | null;
};

function formatTeamAward(award: AwardFull): string {
  const teamRow = award.winners.find((w) => w.teamId !== null);
  const players = award.winners.filter(
    (w) => w.playerId !== null && w.teamId === null,
  );

  const teamLine = teamRow?.team
    ? `**${teamRow.team.name}**`
    : "_Equipo desconocido_";
...
export default {
  ...
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      "Palmarés completo de una temporada: títulos y premios individuales.",
    )
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      if (!season || season.modalityId !== modality.id) {
        await interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setColor(0xe74c3c)
              .setDescription("Temporada no encontrada."),
          ],
        });
        return;
      }
        ...
        await interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setColor(0xe67e22)
              .setDescription(
                `No hay premios registrados en **${season.name}** · **${modality.displayName}**.`,
              ),
          ],
        });
        return;
      ...
      logger.error(
        `/league-history | modalidad: ${modalityName} | temporada: ${seasonIdRaw}`,
        error,
      );
      await interaction.editReply({
        embeds: [
          new EmbedBuilder()
            .setColor(0xe74c3c)
            .setDescription(
              "No se pudo cargar el palmarés. El equipo técnico fue notificado.",
            ),
        ],
      });
    }
