...
function isDTorSubDT(member: GuildMember, settings: ModalitySettings): boolean {
    ...
    if (settings.rol_sub_dt && member.roles.cache.has(settings.rol_sub_dt)) return true;
    return false;
}
...
export default {
    ...
    data: new SlashCommandBuilder()
        ...
        .addStringOption(opt =>
            opt.setName("modalidad").setDescription("Modalidad de tu equipo.").setRequired(true).setAutocomplete(true)
        )
        .addStringOption(opt =>
            opt.setName("uniforme_local").setDescription("Uniforme local. Debe iniciar con /colors.")
        )
        .addStringOption(opt =>
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        const uniVisit = interaction.options.getString("uniforme_visitante")?.trim();

        if (!member) {
            await interaction.editReply({ content: "No se pudo verificar tu identidad en el servidor." });
            return;
        }

        if (!uniLocal && !uniVisit) {
            await interaction.editReply({
                content: "Debes indicar al menos un uniforme (`uniforme_local` o `uniforme_visitante`).",
            });
            return;
        }

        if (uniLocal && !REGEX_COLORS.test(uniLocal)) {
            await interaction.editReply({ content: "El uniforme local debe iniciar con `/colors`." });
            return;
        }
        if (uniVisit && !REGEX_COLORS.test(uniVisit)) {
            await interaction.editReply({ content: "El uniforme visitante debe iniciar con `/colors`." });
            return;
        }
        ...
        try {
            ...
            if (!isDTorSubDT(member, settings)) {
                await interaction.editReply({
                    embeds: [new EmbedBuilder().setColor(0xE74C3C).setTitle("Sin permisos")
                        .setDescription("Solo los **Directores Técnicos** y **Sub-Directores Técnicos** pueden editar los uniformes de su equipo.")],
                });
                return;
            }
            ...
            const team = await Team.findOne({ where: { id: participant.teamId, isActive: true } });
            if (!team) {
            ...
            if (cambios.length === 0) {
                await interaction.editReply({ content: "Los uniformes indicados son iguales a los actuales. Sin cambios." });
                return;
            }
            ...
            await interaction.editReply({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setDescription(`Los uniformes de **${team.name}** han sido actualizados.`)
                        .addFields(
                            { name: "🏠 Local", value: `\`${team.uniformHome ?? "N/A"}\``, inline: false },
                            { name: "✈️ Visita", value: `\`${team.uniformAway ?? "N/A"}\``, inline: false },
                        )
                        .setFooter({ text: `iDinox v3 · ${modality.displayName} · ${season.name}` })
                        .setTimestamp(),
                ],
            });
            ...
            await interaction.editReply({
                embeds: [new EmbedBuilder().setColor(0xE74C3C).setTitle("Algo salió mal")
                    .setDescription("No se pudo actualizar los uniformes. El equipo técnico fue notificado.")],
            });
        }
    },
};