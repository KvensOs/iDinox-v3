...
export default {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        .setName("add")
        .setDescription(
          "Añade o resta estadísticas a un jugador en una competición.",
        )
        .addStringOption((opt) =>
          opt
            .setName("modalidad")
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    const competitionId = Number(
      interaction.options.getString("competicion", true),
    );
    ...
    try {
      ...
      if (!season) {
        await interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setColor(0xe67e22)
              .setDescription("No hay temporada activa en esta modalidad."),
          ],
        });
        return;
      }
      ...
      if (!competition) {
        await interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setColor(0xe74c3c)
              .setDescription("Competición no encontrada o inactiva."),
          ],
        });
        return;
      }
      ...
      if (!targetPlayer) {
        await interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setColor(0xe74c3c)
              .setDescription("Jugador no encontrado en iDinox."),
          ],
        });
        return;
      }
      ...
      if (!Object.keys(deltas).length) {
        await interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setColor(0xe67e22)
              .setDescription("No se detectaron stats válidas en el input."),
          ],
        });
        return;
      }
        ...
        await interaction.editReply({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                result.errorMessage ??
                  "No se pudieron aplicar las estadísticas.",
              ),
          ],
        });
        return;
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: "Cambios",
            value: result.appliedChanges.join("\n") || "_Ninguno_",
          },
        )
        .setTimestamp();
      ...
      await interaction.editReply({
        embeds: [
          new EmbedBuilder()
            .setColor(0xe74c3c)
            .setDescription(
              "Ocurrió un error al procesar las estadísticas. El equipo técnico fue notificado.",
            ),
        ],
      });
    }
