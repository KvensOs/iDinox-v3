...
async function loadRows(
  ...
  const statWhere: Record<string, unknown> = {
  ...
  for (const s of stats) {
    const current = statMap.get(s.participantId) ?? {
      goles: 0,
      asistencias: 0,
      vallas: 0,
      autogoles: 0,
  ...
  for (const p of participants) {
    ...
    const player = (p as unknown as { player: Player; team: Team | null })
      .player;
  ...
  if (view === "overview") {
    for (const sk of STAT_KEYS) {
      const sorted = [...rows].sort(
        (a, b) => (b.stats[sk.id] ?? 0) - (a.stats[sk.id] ?? 0),
      );
      ...
      embed.addFields({
        ...
        value: top
          ? `${prefix}<@${top.discordId}> — **${top.stats[sk.id]}**`
          : "_Sin datos_",
        inline: true,
      });
    }
...
function buildComponents(
  ...
  const catMenu = new StringSelectMenuBuilder()
    ...
    .addOptions([
      new StringSelectMenuOptionBuilder()
        .setLabel("Vista general")
        .setValue("overview")
        .setEmoji("🏠"),
      ...STAT_KEYS.map((sk) =>
        new StringSelectMenuOptionBuilder()
          .setLabel(sk.label)
          .setValue(sk.id as string)
          .setEmoji(sk.emoji),
      ),
    ]);
  rows.push(
  ...
  if (state.allSeasons.length > 1) {
    ...
    rows.push(
      new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(seasonMenu),
    );
  }
  ...
  if (state.competitions.length > 0) {
    ...
    rows.push(
      new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(compMenu),
    );
  }
        ...
        await interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setColor(0xe67e22)
              .setDescription(
                `No hay estadísticas registradas en **${modality.displayName}** esta temporada.`,
              ),
          ],
        });
        return;
      ...
      collector.on("collect", async (i) => {
        if (i.customId === "select_stat") {
          ...
          if (val === "all") {
            ...
            const comp =
              state.competitions.find((c) => c.id === Number(val)) ?? null;
      ...
      await interaction.editReply({
        embeds: [
          new EmbedBuilder()
            .setColor(0xe74c3c)
            .setDescription(
              "No se pudo cargar el ranking. El equipo técnico fue notificado.",
            ),
        ],
      });
    }
