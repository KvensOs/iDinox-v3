...
export default {
    ...
    data: new SlashCommandBuilder()
        ...
        .setDescription("Verifica el estado de un jugador en las modalidades activas.")
        .setDMPermission(false)
        .addUserOption(opt =>
            opt.setName("jugador").setDescription("Jugador a verificar (por defecto: tú).").setRequired(false)
        )
        .addStringOption(opt =>
            opt.setName("modalidad").setDescription("Filtrar por modalidad (opcional).").setRequired(false).setAutocomplete(true)
        ),
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        if (targetUser.bot) {
            await interaction.editReply({ content: "Los bots no tienen ficha en iDinox." });
            return;
        }
        ...
        try {
            ...
            const modalities = await Modality.findAll({
                where: { isActive: true, ...(modalityName ? { name: modalityName } : {}) },
            });
            ...
            const player = await Player.findOne({ where: { discordId: targetUser.id } });
            ...
            for (const modality of modalities) {
                ...
                const team = participant.teamId ? await Team.findByPk(participant.teamId) : null;
                const equipoStr = team ? `[${team.abbreviation}] ${team.name}` : "Agente libre";
            ...
            await interaction.editReply({
                embeds: [new EmbedBuilder().setColor(0xE74C3C)
                    .setDescription("No se pudo verificar al jugador. El equipo técnico fue notificado.")],
            });
        }
    },
};