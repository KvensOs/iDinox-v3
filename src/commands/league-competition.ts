...
import { Competition, CompetitionType } from "../database/models/Competition.js";
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub =>
            sub.setName("new")
                ...
                .addStringOption(opt =>
                    opt.setName("nombre").setDescription("Nombre de la competencia.").setRequired(true)
                        .setMinLength(3).setMaxLength(120)
                )
                .addStringOption(opt =>
        ...
        .addSubcommand(sub =>
            sub.setName("edit")
                ...
                .addStringOption(opt =>
                    opt.setName("competencia").setDescription("Competencia a editar.").setRequired(true).setAutocomplete(true)
                )
                .addStringOption(opt =>
                    opt.setName("nombre").setDescription("Nuevo nombre.").setMinLength(3).setMaxLength(120)
                )
                .addStringOption(opt =>
        ...
        .addSubcommand(sub =>
            sub.setName("close")
                ...
                .addStringOption(opt =>
                    opt.setName("competencia").setDescription("Competencia a cerrar.").setRequired(true).setAutocomplete(true)
                )
        )
        ...
        .addSubcommand(sub =>
            sub.setName("delete")
                ...
                .addStringOption(opt =>
                    opt.setName("competencia").setDescription("Competencia a eliminar.").setRequired(true).setAutocomplete(true)
                )
        ),
            ...
            if (!season) {
                await interaction.editReply({
                    embeds: [new EmbedBuilder().setColor(0xE67E22).setTitle("Sin temporada activa")
                        .setDescription(`No hay una temporada activa en **${modality.displayName}**. Crea una primero.`)],
                });
                return;
            }

            if (sub === "new") return await handleNew(interaction, modality, season);
            if (sub === "edit") return await handleEdit(interaction, modality, season);
            if (sub === "close") return await handleClose(interaction, modality, season);
            if (sub === "delete") return await handleDelete(interaction, modality, season);
...
async function handleEdit(
    ...
    if (!competition) {
        await interaction.editReply({ content: `No se encontró la competencia **${compName}** en la temporada activa.` });
        return;
    }
    ...
    if (newTipo && newTipo !== competition.type) {
        cambios.push(`Tipo: \`${getTypeLabel(competition.type)}\` → \`${getTypeLabel(newTipo)}\``);
...
async function handleClose(
    ...
    if (!competition.isActive) {
        await interaction.editReply({ content: `**${competition.name}** ya está cerrada.` });
        return;
    }
    ...
    await interaction.editReply({
        embeds: [
            new EmbedBuilder()
                ...
                .setFooter({ text: `iDinox v3 · Cerrada por ${interaction.user.username}` })
                .setTimestamp(),
        ],
    });
}
...
async function handleDelete(
    ...
    const totalStats = await Stat.count({ where: { competitionId: competition.id } });
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setCustomId("confirm_delete_comp").setLabel("Eliminar definitivamente").setStyle(ButtonStyle.Danger),
        new ButtonBuilder().setCustomId("cancel_delete_comp").setLabel("Cancelar").setStyle(ButtonStyle.Secondary),
    );
