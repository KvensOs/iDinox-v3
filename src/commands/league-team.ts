...
async function downloadLogo(url: string, filename: string): Promise<string> {
    ...
    if (!res.ok) throw new Error(`No se pudo descargar el logo: ${res.statusText}`);
...
async function deleteLogo(logoPath: string | null): Promise<void> {
    if (!logoPath) return;
    const fullPath = path.isAbsolute(logoPath) ? logoPath : path.join(LOGOS_DIR, logoPath);
...
function validateUniform(value: string | null | undefined, label: string): string | null {
...
async function loadActiveParticipants(teamId: number): Promise<ParticipantWithPlayer[]> {
...
async function refreshTeamNicknames(
    ...
    await Promise.allSettled(
        participants.map(p => {
            const isDT = !!(settings.rol_dt && p.player.discordId &&
                guild.members.cache.get(p.player.discordId)?.roles.cache.has(settings.rol_dt));
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub =>
            sub.setName("add")
                ...
                .addRoleOption(opt =>
                    opt.setName("rol").setDescription("Rol de Discord del equipo.").setRequired(true)
                )
                .addStringOption(opt =>
                    opt.setName("abreviacion").setDescription("Abreviación del equipo (máx. 5 letras).").setRequired(true)
                        .setMinLength(2).setMaxLength(5)
                )
                .addAttachmentOption(opt =>
                    opt.setName("logo").setDescription("Logo oficial del equipo (imagen).").setRequired(true)
                )
                .addStringOption(opt =>
                    opt.setName("uniforme_local").setDescription("Uniforme local. Debe iniciar con /colors.").setRequired(true)
                )
                .addUserOption(opt =>
                    opt.setName("dt").setDescription("Director Técnico del equipo.").setRequired(true)
                )
                .addStringOption(opt =>
                    opt.setName("nombre").setDescription("Nombre oficial (por defecto usa el nombre del rol).")
                )
                .addStringOption(opt =>
                    opt.setName("uniforme_visitante").setDescription("Uniforme visitante. Debe iniciar con /colors.")
                )
        )
        ...
        .addSubcommand(sub =>
            sub.setName("edit")
                ...
                .addStringOption(opt =>
                    opt.setName("equipo").setDescription("Abreviación del equipo.").setRequired(true).setAutocomplete(true)
                )
                .addStringOption(opt => opt.setName("nombre").setDescription("Nuevo nombre oficial."))
                .addStringOption(opt =>
                    opt.setName("abreviacion").setDescription("Nueva abreviación (máx. 5 letras).")
                        .setMinLength(2).setMaxLength(5)
                )
                .addRoleOption(opt => opt.setName("rol").setDescription("Nuevo rol de Discord."))
                .addAttachmentOption(opt => opt.setName("logo").setDescription("Nuevo logo oficial (imagen)."))
                .addStringOption(opt => opt.setName("uniforme_local").setDescription("Nuevo uniforme local (/colors)."))
                .addStringOption(opt => opt.setName("uniforme_visitante").setDescription("Nuevo uniforme visitante (/colors)."))
        )
    ...
    async autocomplete(interaction: AutocompleteInteraction): Promise<void> {
        ...
        if (focused.name === "equipo") {
            ...
            return interaction.respond(
                teams.map(t => ({ name: `🛡️ ${t.name} [${t.abbreviation}]`, value: t.abbreviation }))
            );
        }
    },
        ...
        try {
            ...
            await interaction.editReply({
                embeds: [new EmbedBuilder().setColor(0xE74C3C).setTitle("Algo salió mal")
                    .setDescription("No se pudo completar la operación. El equipo técnico fue notificado.")],
            });
        }
    },
};
...
async function handleAdd(interaction: ChatInputCommandInteraction, modality: Modality): Promise<void> {
    ...
    const abreviacion = interaction.options.getString("abreviacion", true).toUpperCase().trim();
    ...
    const uniVisit = interaction.options.getString("uniforme_visitante")?.trim() ?? null;

    if (!logoAttach.contentType?.startsWith("image/")) {
        await interaction.editReply({ content: "El logo debe ser una imagen (jpg, png, webp)." });
        return;
    }

    const uniError = validateUniform(uniLocal, "local") ?? validateUniform(uniVisit, "visitante");
    if (uniError) {
    ...
    const duplicado = await Team.findOne({
        where: {
            modalityId: modality.id,
            [Op.or]: [{ name: nombre }, { abbreviation: abreviacion }, { roleId: rol.id }],
        },
    });
    if (duplicado) {
        await interaction.editReply({ content: "Ya existe un equipo con ese nombre, abreviación o rol en esta modalidad." });
        return;
    }

    const season = await Season.findOne({ where: { modalityId: modality.id, isActive: true } });
    if (!season) {
        await interaction.editReply({ content: "No hay una temporada activa en esta modalidad." });
        return;
    }
    ...
    const [dtPlayer] = await Player.findOrCreate({
        ...
        defaults: { discordId: dtUser.id, username: dtUser.username, globalName: dtUser.globalName ?? null },
    });

    const [dtParticipant, created] = await Participant.findOrCreate({
        where: { playerId: dtPlayer.id, seasonId: season.id, modalityId: modality.id },
        defaults: { playerId: dtPlayer.id, seasonId: season.id, modalityId: modality.id, teamId: team.id, isActive: true },
    });
    ...
    await interaction.editReply({
        embeds: [
            new EmbedBuilder()
                ...
                .setDescription(`**${nombre}** ha sido añadido a **iDinox ${modality.displayName}**.`)
                ...
                .addFields(
                    ...
                    { name: "Uniformes", value: `🏠 \`${uniLocal}\`\n✈️ \`${uniVisit ?? "N/A"}\`` },
                )
                .setFooter({ text: `iDinox v3 · Registrado por ${interaction.user.username}` })
                .setTimestamp(),
        ],
...
async function handleEdit(interaction: ChatInputCommandInteraction, modality: Modality): Promise<void> {
    const equipoInput = interaction.options.getString("equipo", true).toUpperCase();
    const nuevoNombre = interaction.options.getString("nombre")?.trim();
    const nuevaAbrev = interaction.options.getString("abreviacion")?.toUpperCase().trim();
    ...
    const nuevoUniAway = interaction.options.getString("uniforme_visitante")?.trim() ?? null;

    const uniError = validateUniform(nuevoUniHome, "local") ?? validateUniform(nuevoUniAway, "visitante");
    if (uniError) {
        await interaction.editReply({ content: uniError });
        return;
    }

    const team = await Team.findOne({ where: { modalityId: modality.id, abbreviation: equipoInput, isActive: true } });
    if (!team) {
        await interaction.editReply({ content: `No se encontró el equipo **${equipoInput}** en ${modality.displayName}.` });
        return;
    }
    ...
    if (nuevaAbrev && nuevaAbrev !== team.abbreviation) {
        ...
        await refreshTeamNicknames(interaction.guild!, participants, nuevaAbrev, settings);
    ...
    await interaction.editReply({
        embeds: [
            new EmbedBuilder()
                ...
                .setDescription(`**${team.name}** ha sido modificado en **iDinox ${modality.displayName}**.`)
                .addFields({ name: "Cambios", value: cambios.join("\n") })
                .setFooter({ text: `iDinox v3 · Editado por ${interaction.user.username}` })
                .setTimestamp(),
        ],
    });
}
...
async function handleDelete(interaction: ChatInputCommandInteraction, modality: Modality): Promise<void> {
    ...
    const confirmEmbed = new EmbedBuilder()
        ...
        .setDescription("Esta acción es irreversible. El equipo será desactivado y todos sus jugadores quedarán como agentes libres.")
        .addFields(
            { name: "Equipo", value: `\`${team.name}\` [\`${team.abbreviation}\`]`, inline: true },
            { name: "Rol", value: `<@&${team.roleId}>`, inline: true },
            { name: "Jugadores", value: `\`${totalJugadores}\` serán liberados`, inline: true },
        )
        .setFooter({ text: "Tienes 20 segundos para confirmar." });

    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setCustomId("confirm_delete").setLabel("Eliminar definitivamente").setStyle(ButtonStyle.Danger),
    ...
    collector.on("collect", async i => {
        ...
        await Promise.allSettled(
            participants.map(p => {
                ...
                return interaction.guild!.members.fetch(p.player.discordId)
