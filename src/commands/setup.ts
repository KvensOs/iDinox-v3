...
function buildOverviewEmbed(
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: "Mercado",
        value: settings.marketOpen ? "✅ Abierto" : "❌ Cerrado",
        inline: true,
      },
    )
...
export default {
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt
        .setName("campo")
        .setDescription(
          "Campo a configurar. Omítelo para ver el estado actual.",
        )
        .setRequired(false)
        .addChoices(
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    const campo = interaction.options.getString("campo") as
      | keyof Omit<ModalitySettings, "marketOpen">
      | null;
    ...
    try {
        ...
        await interaction.editReply({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                `La modalidad **${modalityName.toUpperCase()}** no existe en el sistema.`,
              ),
          ],
        });
        return;
      ...
      if (!campo) {
        await interaction.editReply({
          embeds: [buildOverviewEmbed(modality, settings)],
        });
        return;
      }
      ...
      if (fieldMeta.type === "role" && !rol) {
        await interaction.editReply({
          content:
            "Este campo requiere un **rol**. Usa la opción `rol` al ejecutar el comando.",
        });
        return;
      }
      ...
      if (fieldMeta.type === "channel" && !canal) {
        await interaction.editReply({
          content:
            "Este campo requiere un **canal**. Usa la opción `canal` al ejecutar el comando.",
        });
        return;
      }
      ...
      const oldDisplay = oldValue
        ...
        : "`Sin configurar`";
      const newDisplay =
        fieldMeta.type === "role" ? `<@&${newValue}>` : `<#${newValue}>`;
      ...
      await interaction.editReply({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              `**${fieldMeta.label}** actualizado en **iDinox ${modality.displayName}**.`,
            )
            .addFields(
            ...
            .setFooter({
              text: `iDinox v3 · Configurado por ${interaction.user.username}`,
            })
            .setTimestamp(),
        ],
      });
      ...
      await interaction.editReply({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              "No se pudo guardar la configuración. El equipo técnico fue notificado.",
            ),
        ],
      });
    }
