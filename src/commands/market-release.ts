...
function buildPublicEmbed(d: ReleaseEmbedData): EmbedBuilder {
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: "👥 Plantilla",
        value: `\`${d.plantilla}/${d.modality.playersPerTeam}\``,
        inline: true,
      },
    ...
    .setFooter({
      text: `iDinox v3 · ${d.modality.displayName} · ${d.season.name}`,
    })
    .setTimestamp();
...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (targetUser.bot) {
      await interaction.editReply({
        content: "No puedes dar de baja a un bot.",
      });
      return;
    }
    if (targetUser.id === interaction.user.id) {
      await interaction.editReply({
        content: "No puedes darte de baja a ti mismo.",
      });
      return;
    }
    ...
    try {
        ...
        await interaction.editReply({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                "Solo los **Directores Técnicos** y **Sub-DT** pueden dar de baja jugadores.",
              ),
          ],
        });
        return;
        ...
        await interaction.editReply({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                `**${targetUser.globalName ?? targetUser.username}** no tiene ficha en iDinox.`,
              ),
          ],
        });
        return;
      ...
      if (
        !targetParticipant?.teamId ||
        targetParticipant.teamId !== dtParticipant.teamId
      ) {
        await interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setColor(0xe74c3c)
              .setTitle("Jugador no pertenece a tu equipo")
              .setDescription(
      ...
      const color =
        rolEquipo?.color && rolEquipo.color !== 0 ? rolEquipo.color : 0xed4245;
      ...
      if (fondoExists) {
        files.push(
          new AttachmentBuilder(FONDO_PATH, { name: "fondo_bajas.png" }),
        );
      }

      const canal = settings.canal_mercado_bajas
        ? (interaction.guild?.channels.cache.get(
            settings.canal_mercado_bajas,
        ...
        embeds: [
          new EmbedBuilder()
            ...
            .addFields(
              ...
              {
                name: "Plantilla",
                value: `\`${embedData.plantilla}/${modality.playersPerTeam}\``,
                inline: true,
              },
            )
            .setTimestamp(),
      ...
      await interaction.editReply({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              "No se pudo completar la baja. El equipo técnico fue notificado.",
            ),
        ],
      });
    }
