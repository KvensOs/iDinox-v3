...
function buildEmbed(
  ...
  const color =
    rol?.hexColor && rol.hexColor !== "#000000" ? rol.hexColor : COLOR_FALLBACK;
  ...
  const listaStr = roster.length
    ? roster
        .map((r, i) => {
          ...
          const posStr =
            r.position && r.position !== "N/A" ? `\`${r.position}\` ` : "";
  ...
  return embed.setFooter({ text: "iDinox v3 · Ficha de Club" }).setTimestamp();
}
    ...
    try {
      ...
      if (equipoInput) {
        team = await Team.findOne({
          where: { id: Number(equipoInput), isActive: true },
        });
      } else {
        ...
        if (player) {
            ...
            await interaction.editReply({
              embeds: [
                new EmbedBuilder()
                  .setColor(0xe67e22)
                  .setDescription(
                    "No perteneces a ningún equipo actualmente. Indica un equipo con el parámetro `equipo`.",
                  ),
              ],
            });
            return;
          ...
          if (withTeam.length === 1) {
            team = await Team.findOne({
              where: { id: withTeam[0].teamId!, isActive: true },
            });
          } else {
            const teams = await Promise.all(
              withTeam.map((p) => Team.findByPk(p.teamId!)),
            );
            const modalities = await Modality.findAll({
              where: { isActive: true },
            });
            const modalityMap = new Map(
              modalities.map((m) => [m.id, m.displayName]),
            );
            ...
            const opciones = teams
              ...
              .map(
                (t) =>
                  `· **${t!.name}** \`${modalityMap.get(t!.modalityId) ?? "?"}\``,
              )
              .join("\n");
        ...
        await interaction.editReply({
          embeds: [
            new EmbedBuilder()
              .setColor(0xe74c3c)
              .setDescription(
                "No se encontró el equipo o no estás en ninguno.",
              ),
          ],
        });
        return;
      ...
      const modality = await Modality.findOne({
        where: { id: team.modalityId, isActive: true },
      });
      if (!modality) {
        await interaction.editReply({
          content: "La modalidad de este equipo no está activa.",
        });
        return;
      }
      ...
      if (!season) {
        await interaction.editReply({
          content: `No hay temporada activa en **${modality.displayName}**.`,
        });
        return;
      }

      const logoFilename = team.logoPath ? path.basename(team.logoPath) : null;
      const logoFullPath = logoFilename
        ? path.join(LOGOS_DIR, logoFilename)
        : null;
      const hasLogo = !!(logoFullPath && (await fileExists(logoFullPath)));
      const files = hasLogo
        ? [new AttachmentBuilder(logoFullPath!, { name: "logo.png" })]
        : [];
      ...
      logger.error(
        `/club | user: ${interaction.user.id} | equipo: ${equipoInput}`,
        error,
      );
      await interaction.editReply({
        embeds: [
          new EmbedBuilder()
            .setColor(0xe74c3c)
            .setDescription(
              "No se pudo cargar la ficha del club. El equipo técnico fue notificado.",
            ),
        ],
      });
    }
