...
async function backupDatabase(label: string): Promise<string> {
  ...
  logger.success(`season backup | ${filename} (WAL checkpointed)`);
  return filename;
}
...
async function handleNew(
  ...
  const confirmar =
    interaction.options.getString("confirmar", false)?.trim() ?? null;

  if (!SEASON_SECRET) {
    return void interaction.editReply({
      embeds: [
        err("La variable `SEASON_SECRET` no está configurada en el servidor."),
      ],
    });
  }
  ...
  if (nameConflict) {
    return void interaction.editReply({
      embeds: [
        warn(
          `Ya existe una temporada llamada **"${nombre}"** en **${modality.displayName}**.`,
        ),
      ],
    });
  }
  ...
  const backupFile = await backupDatabase(
    `before_new_${nombre.replace(/\s+/g, "_")}`,
  );
  ...
  const lines: string[] = [
    ...
    carryCount
      ? `Plantillas copiadas: \`${carryCount}\` participantes`
      : "_Sin plantillas anteriores_",
    backupFile ? `Backup: \`${backupFile}\`` : "",
  ].filter(Boolean);

  return void interaction.editReply({
    embeds: [
      ok("✅ Nueva temporada iniciada", lines.join("\n")).setFooter({
        text: `Por ${interaction.user.username} · iDinox v3`,
      }),
    ],
  });
}
...
async function handleEnd(
  ...
  const backupFile = await backupDatabase(
    `before_end_${season.name.replace(/\s+/g, "_")}`,
  );
  ...
  const lines = [
    ...
    compsActivas
      ? `Competencias cerradas: \`${compsActivas}\``
      : "_Sin competencias activas_",
    backupFile ? `Backup: \`${backupFile}\`` : "",
  ].filter(Boolean);

  return void interaction.editReply({
    embeds: [
      ok("🔒 Temporada cerrada", lines.join("\n")).setFooter({
        text: `Por ${interaction.user.username} · iDinox v3`,
      }),
    ],
  });
...
async function handleEdit(
  ...
  const nuevoNombre = interaction.options
    .getString("nuevo_nombre", true)
    .trim();
  ...
  if (!season) {
    return void interaction.editReply({
      embeds: [err("Temporada no encontrada.")],
    });
  }
  ...
  if (conflict) {
    return void interaction.editReply({
      embeds: [
        warn(
          `Ya existe una temporada llamada **"${nuevoNombre}"** en **${modality.displayName}**.`,
        ),
      ],
    });
  }
  ...
  logger.success(
    `/season edit | "${nombreAnterior}" → "${nuevoNombre}" | ${modality.displayName} | ${interaction.user.username}`,
  );

  return void interaction.editReply({
    embeds: [
      ok(
        "✏️ Temporada actualizada",
...
async function handleInfo(
  ...
  const compLines =
    [
      ...(activeComps.length
        ? activeComps.map((c) => `· 🟢 \`${c.name}\` _(${c.type})_`)
        : []),
      ...(inactiveComps.length
        ? inactiveComps.map((c) => `· 🔴 \`${c.name}\` _(${c.type})_`)
        : []),
    ].join("\n") || "_Sin competencias_";
...
export default {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        .setName("new")
        .setDescription(
          "Crea una nueva temporada y cierra la anterior (si existe).",
        )
        .addStringOption((opt) =>
          opt
            .setName("modalidad")
        ...
        .addStringOption((opt) =>
          opt
            .setName("clave")
            .setDescription("Clave de seguridad (SEASON_SECRET).")
            .setRequired(true),
        )
        .addStringOption((opt) =>
          opt
            .setName("confirmar")
            .setDescription(
              "Escribe el nombre exacto de la temporada activa para confirmar su cierre.",
            )
            .setRequired(false),
        ),
    )
      ...
      sub
        ...
        .addStringOption((opt) =>
          opt
            .setName("confirmar")
            .setDescription(
              "Escribe el nombre exacto de la temporada activa para confirmar.",
            )
            .setRequired(true),
        ),
    )
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((opt) =>
          opt
            .setName("temporada")
            .setDescription("Temporada a editar.")
            .setRequired(true)
            .setAutocomplete(true),
        )
        .addStringOption((opt) =>
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      switch (sub) {
        case "new":
          return void (await handleNew(interaction, modality));
        case "end":
          return void (await handleEnd(interaction, modality));
        case "edit":
          return void (await handleEdit(interaction, modality));
        case "info":
          return void (await handleInfo(interaction, modality));
      }
    } catch (error) {
      logger.error(
        `/season ${sub} | user: ${interaction.user.id} | modalidad: ${modalityName}`,
        error,
      );
      return void interaction.editReply({
        embeds: [
