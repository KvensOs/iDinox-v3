...
async function respondTeams(
  ...
  const activeTeamIds = await Participant.findAll({
    ...
    rows.map((r) => r.teamId).filter((id): id is number => id !== null),
  );
...
async function handleAdd(
  ...
  if (tipo === "team" && !equipoIdRaw) {
    return void interaction.editReply({
      embeds: [
        err(
          "Para un premio de **equipo** debes indicar el parámetro `equipo`.",
        ),
      ],
    });
  }
  if (tipo === "individual" && !jugadorUser) {
    return void interaction.editReply({
      embeds: [
        err(
          "Para un premio **individual** debes indicar el parámetro `jugador`.",
        ),
      ],
    });
  }
  ...
  if (!season) {
    return void interaction.editReply({
      embeds: [err(`No hay temporada activa en **${modality.displayName}**.`)],
    });
  }
  ...
  if (competenciaId) {
    competition = await Competition.findOne({
      where: { id: Number(competenciaId), seasonId: season.id },
    });
    if (!competition) {
      return void interaction.editReply({
        embeds: [
          err("La competencia no existe o no pertenece a esta temporada."),
        ],
      });
    }
  }
  ...
  const existing = await Award.findOne({
    where: { seasonId: season.id, name: nombre },
  });
  if (existing) {
    return void interaction.editReply({
      embeds: [
        warn(
          `Ya existe un premio **"${nombre}"** en la temporada \`${season.name}\`.`,
        ),
      ],
    });
  }
  ...
  if (tipo === "team") {
    ...
    if (!team) {
      await award.destroy();
      return void interaction.editReply({
        embeds: [err("El equipo no existe o no pertenece a esta modalidad.")],
      });
    }
    ...
    logger.success(
      `/league-trophy add | TEAM | "${nombre}" → ${team.name} | ${season.name} | ${modality.displayName} | ${interaction.user.username}`,
    );
  ...
  const player = await Player.findOne({
    where: { discordId: jugadorUser!.id },
  });
  if (!player) {
    await award.destroy();
    return void interaction.editReply({
      embeds: [
        err(`**${jugadorUser!.username}** no está registrado en iDinox.`),
      ],
    });
  }
  ...
  if (!participant) {
    ...
    return void interaction.editReply({
      embeds: [
        err(
          `<@${jugadorUser!.id}> no tiene ficha en **${modality.displayName}** · \`${season.name}\`.`,
        ),
      ],
    });
  }
  ...
  logger.success(
    `/league-trophy add | INDIVIDUAL | "${nombre}" → ${player.username} | ${season.name} | ${modality.displayName} | ${interaction.user.username}`,
  );
  ...
  return void interaction.editReply({
    embeds: [
      ok(
        "🎖️ Premio individual registrado",
...
async function handleEdit(
  ...
  if (!nuevoNombre && nuevasNotas === null && competenciaId === null) {
    return void interaction.editReply({
      embeds: [
        warn(
          "Indica al menos un campo a editar: `nuevo_nombre`, `notas` o `competencia`.",
        ),
      ],
    });
  }
  ...
  if (!award) {
    return void interaction.editReply({
      embeds: [err("Premio no encontrado o no pertenece a esta modalidad.")],
    });
  }
  ...
  if (competenciaId) {
    ...
    if (!competition) {
      return void interaction.editReply({
        embeds: [
          err(
            "La competencia no existe o no pertenece a la temporada del premio.",
          ),
        ],
      });
    }
  ...
  if (nuevoNombre && nuevoNombre !== award.name) {
    const conflict = await Award.findOne({
      where: { seasonId: season.id, name: nuevoNombre },
    });
    if (conflict) {
      return void interaction.editReply({
        embeds: [
          warn(
            `Ya existe un premio **"${nuevoNombre}"** en la temporada \`${season.name}\`.`,
          ),
        ],
      });
    }
  ...
  logger.success(
    `/league-trophy edit | #${award.id} "${award.name}" | ${modality.displayName} | ${interaction.user.username}`,
  );

  return void interaction.editReply({
    embeds: [
      ok(
        "✏️ Premio actualizado",
        `**${award.name}**\n🗓️ Temporada: \`${season.name}\` · \`${modality.displayName}\``,
      ).setFooter({ text: `Por ${interaction.user.username} · iDinox v3` }),
    ],
  });
...
async function handleDelete(
  ...
  const award = (await findAward(Number(awardIdRaw), modality.id)) as
    | (Award & { season: Season })
    | null;
  if (!award) {
    return void interaction.editReply({
  ...
  logger.success(
    `/league-trophy delete | #${award.id} "${nombre}" (${winnersN} winners) | ${season.name} | ${modality.displayName} | ${interaction.user.username}`,
  );
...
export default {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((opt) =>
          opt
            .setName("equipo")
            .setDescription("Equipo ganador (requerido si tipo = equipo).")
            .setRequired(false)
            .setAutocomplete(true),
        )
        .addUserOption((opt) =>
          opt
            .setName("jugador")
            .setDescription("Jugador ganador (requerido si tipo = individual).")
            .setRequired(false),
        )
        .addStringOption((opt) =>
          opt
            .setName("competencia")
            .setDescription("Competencia asociada (opcional).")
            .setRequired(false)
            .setAutocomplete(true),
        )
        .addStringOption((opt) =>
    ...
    .addSubcommand((sub) =>
      sub
        .setName("edit")
        .setDescription(
          "Edita nombre, notas o competencia de un premio existente.",
        )
        .addStringOption((opt) =>
          opt
            .setName("modalidad")
        ...
        .addStringOption((opt) =>
          opt
            .setName("premio")
            .setDescription("Premio a editar.")
            .setRequired(true)
            .setAutocomplete(true),
        )
        .addStringOption((opt) =>
          opt
            ...
            .setDescription("Nuevo nombre del premio.")
            .setRequired(false)
            .setMaxLength(120),
        )
        .addStringOption((opt) =>
          opt
            .setName("notas")
            .setDescription("Nuevas notas (dejar vacío para eliminar).")
            .setRequired(false)
            .setMaxLength(500),
        )
        .addStringOption((opt) =>
          opt
            .setName("competencia")
            .setDescription("Nueva competencia asociada.")
            .setRequired(false)
            .setAutocomplete(true),
        ),
    )
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((opt) =>
          opt
            .setName("premio")
            .setDescription("Premio a eliminar.")
            .setRequired(true)
            .setAutocomplete(true),
        ),
    ),
  ...
  async autocomplete(interaction: AutocompleteInteraction): Promise<void> {
    ...
    if (focused.name === "competencia") {
      ...
      return interaction.respond(
        competitions.map((c) => ({ name: c.name, value: String(c.id) })),
      );
    }
  },
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      if (!modality) {
        return void interaction.editReply({
          embeds: [err("Modalidad no encontrada o inactiva.")],
        });
      }
      ...
      switch (sub) {
        case "add":
          return void (await handleAdd(interaction, modality));
        case "edit":
          return void (await handleEdit(interaction, modality));
        case "delete":
          return void (await handleDelete(interaction, modality));
      }
    } catch (error) {
      logger.error(
        `/league-trophy ${sub} | user: ${interaction.user.id} | modalidad: ${modalityName}`,
        error,
      );
      return void interaction.editReply({
        embeds: [
