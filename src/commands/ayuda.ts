...
function buildCategoryMenu(categories: string[], selected: string | null = null) {
...
function buildCategoryEmbed(category: string, commands: CommandMeta[]) {
    ...
    return new EmbedBuilder()
        ...
        .setFooter({ text: `${commands.length} comando${commands.length !== 1 ? "s" : ""} en esta categoría` })
        .setTimestamp();
}
...
export default {
    ...
    data: new SlashCommandBuilder()
        .setName("ayuda")
        .setDescription("Centro de ayuda interactivo con todos los comandos de iDinox.")
    ...
    async execute(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        const categories = [
            CAT_TODOS,
            ...[...categorySet].filter(c => c !== CAT_TODOS && c !== CAT_OTROS).sort(),
        ...
        collector.on("collect", async i => {
            ...
            if (i.customId === "ayuda_category" && i.isStringSelectMenu()) {
                ...
                const cmds = selected === CAT_TODOS
                    ? allCommands
                    : (commandMap.get(selected) ?? []);

                await i.update({
                    embeds: [buildCategoryEmbed(selected, cmds)],
                    components: [buildCategoryMenu(categories, selected), buildHomeButton(false)],
                });
            }
        });
